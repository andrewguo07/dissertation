...
\sect{Protocol for fast fanout using long-range interactions}To perform a fanout gate on $n$ logical qubits, we
...
\begin{figure}[t]
  \begin{centering}
    \includegraphics[scale=0.45]{fanoutgate.pdf}
  \par\end{centering}
...
\sect{Intractability of classical simulation of strongly long-range systems}As
...
\sect{Lower bounds on the time required to implement QFT and fanout}In previous sections, we found upper bounds to the time required to implement fanout---and by corollary, the QFT.
...
\sect{Conclusions and Outlook}In
